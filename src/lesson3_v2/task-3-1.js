...
class Bank extends EventEmitter {
    ...
    init() {
        ...
        this.on('changeLimit', (personId, limit) => {
            ...
            const { index } = this._getCustomerById(personId);

            this.customers[index].limit = limit;
        });
    ...
    _updateBalance({ customer, index, balance, amount }) {
        this._checkLimit({ customer, amount });
        this.customers[index] = { ...customer, balance };
    }

    _increaseBalance({ id, amount }) {
        const { customer, index } = this._getCustomerById(id);

        const balance = customer.balance + amount;
        this._updateBalance({ customer, index, balance, amount });
    }
