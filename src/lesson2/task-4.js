class TimersManager {
    ...
    pause(timerName) {
        ...
        const foundTimer = this._getTimer(timerName);
        const { timerId } = foundTimer;

        if (foundTimer.interval) {
            clearInterval(timerId);
        } else {
            clearTimeout(timerId);
        }

        foundTimer.timerId = null;

        this.timers.map(timer =>
            timer.name === timerName ? foundTimer : timer
        );
    }
    ...
    resume(timerName) {
        ...
        const foundTimer = this._getTimer(timerName);
        const timerId = this._createTimer(foundTimer);

        foundTimer.timerId = timerId;
    ...
    _kill() {
        if (typeof this.timeout !== 'number') {
            throw new Error('manager timeout should be a number');
        }
